"""
pixart_tome_quant_experiment.py
Phase 2: Token Merging + NVFP4_SVDQUANT_DEFAULT_CFG 결합 실험.

Phase 1에서 좋은 결과를 보인 ToMe config와
NVFP4 quantization을 결합하여 최종 speedup + 품질을 평가.

순서:
  1. FP16 reference 생성
  2. NVFP4 quantization (calibration은 ToMe 없이)
  3. ToMe patch 설치 (quantization 이후 — 독립적)
  4. 이미지 생성 + 메트릭 측정
...
CLI:
  ...
  --block_start    0
  --block_end      28
  --sweep                     # 미리 정의된 ToMe+NVFP4 config 전체 sweep
  --lowrank        32
  --num_samples    20
...
결과:
  results/{dataset}/tome_quant/{config_tag}/metrics.json
  results/{dataset}/tome_quant/{config_tag}/metrics.csv
  results/{dataset}/tome_quant/sweep_summary.json  (--sweep 시)
  results/{dataset}/tome_quant/sweep_summary.csv   (--sweep 시)
"""
...
from tome_patch import install_tome, uninstall_tome
from tome_core import compute_r


# ---------------------------------------------------------------------------
# Sweep configs: (merge_ratio, block_start, block_end, tag)
# Phase 1 결과 기반 상위 config
# ---------------------------------------------------------------------------

SWEEP_CONFIGS = [
    (0.0,  0, 28, "NVFP4_only"),          # ToMe 없는 NVFP4 baseline
    (0.20, 0, 28, "NVFP4_R20_ALL"),
    (0.30, 0, 28, "NVFP4_R30_ALL"),
    (0.30, 4, 24, "NVFP4_R30_MID"),
    (0.20, 4, 24, "NVFP4_R20_MID"),
]
...
def generate_and_evaluate(
    ...
    clip_processor=None,
    config_tag: str = "",
    baseline_time_sec: float | None = None,
) -> dict:
    ...
    speedup_vs_baseline = (baseline_time_sec / avg_time) if (baseline_time_sec and avg_time > 0) else None
    ...
    return {
        ...
        "speedup_vs_nvfp4_baseline":  speedup_vs_baseline,
    }
...
CSV_FIELDS = [
    "config_tag", "merge_ratio", "block_start", "block_end", "lowrank",
    "num_steps", "num_samples",
    "fid", "is", "psnr", "ssim", "lpips", "clip",
    "time_per_image_sec", "speedup_vs_nvfp4_baseline",
]
...
def main():
    parser = argparse.ArgumentParser(description="PixArt ToMe + NVFP4 실험")
    ...
    s_count = len(prompts)
    p_count = 2 if args.test_run else min(32, s_count)
    t_count = args.num_inference_steps

    dataset_ref_dir  = os.path.join(args.ref_dir,  args.dataset_name)
    dataset_save_dir = os.path.join(args.save_dir, args.dataset_name, "tome_quant")
    ...
    # -----------------------------------------------------------------------
    if accelerator.is_main_process:
        accelerator.print(f"[ToMe+NVFP4] Generating FP16 reference images...")
    ...
    # -----------------------------------------------------------------------
    # Phase 2: NVFP4 Quantization (ToMe 없이 calibration)
    # -----------------------------------------------------------------------
    accelerator.print(f"\n[ToMe+NVFP4] Quantizing model (lowrank={args.lowrank})...")
    ...
    # -----------------------------------------------------------------------
    # Phase 3: ToMe + NVFP4 sweep
    # -----------------------------------------------------------------------
    configs_to_run = (
        SWEEP_CONFIGS if args.sweep
        else [(args.merge_ratio, args.block_start, args.block_end,
               f"NVFP4_R{int(args.merge_ratio*100)}_s{args.block_start}_e{args.block_end}")]
    )

    sweep_results: list[dict] = []
    nvfp4_only_time: float | None = None  # NVFP4 baseline 시간 (speedup 계산용)
    ...
    for merge_ratio, blk_start, blk_end, config_tag in configs_to_run:
        ...
        metrics = generate_and_evaluate(
            ...
            baseline_time_sec=nvfp4_only_time,
        )

        # NVFP4_only 시간 저장 (speedup 기준점)
        if merge_ratio == 0.0:
            nvfp4_only_time = metrics["time_per_image_sec"]
        ...
        if accelerator.is_main_process:
            ...
            spd = f"{metrics['speedup_vs_nvfp4_baseline']:.3f}x" if metrics["speedup_vs_nvfp4_baseline"] else "N/A"
            accelerator.print(
                f"  FID={metrics['fid']:.2f} | IS={metrics['is']:.3f} | "
                f"PSNR={metrics['psnr']:.2f} | CLIP={metrics['clip']} | "
                f"time={metrics['time_per_image_sec']:.2f}s | "
                f"speedup_vs_nvfp4={spd}"
            )

        # ToMe patch 제거 (다음 config 반복을 위해 원본 forward 복원)
        if merge_ratio > 0.0:
            uninstall_tome(pipe.transformer)
    ...
    if accelerator.is_main_process:
        summary = {
            "method":        "NVFP4_SVDQUANT_DEFAULT_CFG + ToMe",
        ...
        print(f"\n{'=' * 105}")
        print(f"  NVFP4 + ToMe Sweep 완료 | dataset={args.dataset_name} | n={s_count}")
        print(f"{'=' * 105}")
        print(
            f"\n{'config_tag':<24} | {'ratio':>5} | {'blks':>7} | "
            f"{'FID':>8} | {'IS':>7} | {'PSNR':>7} | "
            f"{'sec/img':>8} | {'spd_vs_nvfp4':>12}"
        )
        print("-" * 95)
        for r in sweep_results:
            spd = f"{r['speedup_vs_nvfp4_baseline']:.3f}x" if r["speedup_vs_nvfp4_baseline"] else "    N/A    "
            print(
                f"{r['config_tag']:<24} | {r['merge_ratio']:>5.2f} | "
                f"[{r['block_start']:>2},{r['block_end']:>2}) | "
                f"{r['fid']:>8.2f} | {r['is']:>7.3f} | {r['psnr']:>7.2f} | "
                f"{r['time_per_image_sec']:>8.2f} | {spd:>12}"
            )
        print(f"\n  JSON: {summary_json}")
