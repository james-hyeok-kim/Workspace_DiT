...
# Steps={5,10,15,20} × CacheRange={[8,20),[4,24),[2,26)} × 5 Methods × {none,deepcache,cache_lora}
# Methods: RTN, SVDQUANT, FP4DIT, HQDIT, CONVROT  (PixArt-Sigma, 100 samples)
#
# 실행 순서:
#   Phase 0: none baselines steps={5,10} (모든 method)
#   Phase 1: SVDQUANT 전체 sweep
#   Phase 2: RTN 전체 sweep
#   Phase 3: FP4DIT 전체 sweep
#   Phase 4: HQDIT 전체 sweep
#   Phase 5: CONVROT 전체 sweep
#
# 사용법:
...
# 실험 매트릭스
ALL_METHODS=("RTN" "SVDQUANT" "FP4DIT" "HQDIT" "CONVROT" "SIXBIT")
STEPS=(5 10 15 20)
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --methods)       IFS=',' read -ra FILTER_METHODS <<< "$2"; shift ;;
...
run_one() {
    ...
    RESULT_DIR="/data/james_dit_pixart_sigma_xl_mjhq/${METHOD}/MJHQ/${TAG}"
