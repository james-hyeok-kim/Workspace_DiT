...
5가지 NVFP4/mixed-precision 양자화 방법 구현 (PixArt-Sigma 전용).
...
M3 - (MRGPTQ 제외 — LLM-based quantization)
...
class HQDiTLinear(nn.Module):
    ...
    def __init__(self, original_linear, layer_idx=0, dtype=torch.float16, alpha=0.5):
        super().__init__()
        self.target_dtype = dtype
        self.alpha        = alpha
        self.in_features  = original_linear.in_features
        ...
        # SmoothQuant scale (applied before Hadamard rotation)
        self.register_buffer("smooth_scale",
                             torch.ones(original_linear.in_features, dtype=dtype))
    ...
    @torch.no_grad()
    def calibrate(self, x_max=None):
        """
        SmoothQuant scale (from x_max) → apply before Hadamard rotation.
        Fuse smooth + Hadamard into weight, then per-channel FP4 format selection.
        """
        W = self.weight.float()

        # ── SmoothQuant smooth scale ──────────────────────────────────────────
        if x_max is not None:
            x_max_f = x_max.clamp(min=1e-5).float()
            w_max_f = W.abs().max(dim=0)[0].clamp(min=1e-5)
            smooth = (w_max_f.pow(1 - self.alpha) / x_max_f.pow(self.alpha)).clamp(1e-4, 1e4)
            self.smooth_scale.data = smooth.to(self.target_dtype)
            # Absorb smooth scale into weight columns: W_smooth[i,j] = W[i,j] / smooth[j]
        ...
        # ── Per-output-channel format selection ───────────────────────────────
    ...
    def forward(self, x):
        ...
        # SmoothQuant: scale activation before Hadamard rotation
        x_t = x_t * self.smooth_scale
...
def apply_hqdit_quantization(pipe, transformer, accelerator, prompts, p_count,
    ...
    # Collect x_max via calibration pass (SmoothQuant for Sigma's activation distribution)
    print(f"  [HQDIT] Calibrating ({p_count} prompts)...", flush=True)
    calib_data = _collect_x_max(pipe, transformer, target_names,
                                 prompts, p_count, t_count, device, accelerator)
    ...
    for layer_idx, name in enumerate(tqdm(target_names, desc="[HQDIT] Replacing layers",
        ...
        if next(orig_m.parameters()).device == device:
            ...
            new_layer.calibrate(calib_data.get(name, None))
            set_module_by_name(transformer, name, new_layer)
...
# ---------------------------------------------------------------------------
# M8: ConvRotLinear (Group-wise Regular Hadamard Transform, ConvRot)
# ---------------------------------------------------------------------------

def _build_h64():
    """
    Regular Hadamard H64 via Kronecker product: H4 ⊗ H4 ⊗ H4.
    H4 is a 4×4 Regular Hadamard matrix (balanced ±1).
    Result: 64×64 matrix with entries ±1/√64 = ±0.125.
    """
    H4 = torch.tensor(
        [[1,  1,  1, -1],
         [1,  1, -1,  1],
    ...
    H16 = torch.kron(H4, H4)   # 16×16
    H64 = torch.kron(H16, H4)  # 64×64
    return H64 / (64 ** 0.5)   # normalize so H64 @ H64.T = I


_H64 = _build_h64()  # computed once at import time


def group_rht(x, H, N0=64):
    """
    Group-wise Regular Hadamard Transform.
    x: [..., K]  (K must be divisible by N0)
    H: [N0, N0]  (orthonormal Hadamard matrix)
    Returns: [..., K] — same shape, rotation applied within each group.
    """
    orig_shape = x.shape
    K = orig_shape[-1]
    assert K % N0 == 0, f"K={K} not divisible by N0={N0}"
    x_groups = x.view(*orig_shape[:-1], K // N0, N0)  # [..., K//N0, N0]
    H_dev = H.to(x.device, dtype=x.dtype)
    x_rot = x_groups @ H_dev.T                          # [..., K//N0, N0]
    return x_rot.reshape(orig_shape)


class ConvRotLinear(nn.Module):
    """
    ConvRot: Group-wise Regular Hadamard Transform + SmoothQuant + NVFP4 (W4A4).

    논문: "ConvRot: Accurate W4A4 Quantization for Diffusion Transformers
           via ConvRot-Basis Rotation" (arXiv)

    핵심:
      - Regular Hadamard (H4⊗H4⊗H4 = H64): column discrepancy √n (vs Sylvester n)
        → both row-wise (weight) and column-wise (activation) outlier 억제
      - Group size N0=64 (PixArt hidden_dim=1152, 1152%64=0)
      - SmoothQuant scale을 rotation 전에 적용 (scale을 rotation과 분리)
      - Weight: smooth → group_rht → NVFP4 (offline, inference 시 고정)
      - Activation: smooth → group_rht → NVFP4 (online, runtime)
    """

    N0 = 64  # group size for RHT
    ...
    def __init__(self, original_linear, alpha=0.5, block_size=16, dtype=torch.float16):
        ...
        # Store original weight (for calibrate)
        self.register_buffer("weight",
                             original_linear.weight.data.clone().to(dtype))
        self.bias = nn.Parameter(
            original_linear.bias.data.clone().to(dtype)
        ) if original_linear.bias is not None else None

        # Rotated + quantized weight (filled during calibrate)
        self.register_buffer("w_quantized",
                             original_linear.weight.data.clone().to(dtype))
        ...
        # H64 (fixed, registered so device-moves are automatic)
        self.register_buffer("H64", _H64.clone())

        self.is_calibrated = False

        # Check if in_features is divisible by N0
        if self.in_features % self.N0 != 0:
            raise ValueError(
                f"ConvRotLinear: in_features={self.in_features} "
                f"not divisible by N0={self.N0}"
            )

    @torch.no_grad()
    def calibrate(self, x_max):
        """Compute SmoothQuant scale, rotate weight, then quantize."""
        x_max = x_max.clamp(min=1e-5).float()
        w_max = self.weight.abs().max(dim=0)[0].clamp(min=1e-5).float()
        smooth = (w_max.pow(1 - self.alpha) / x_max.pow(self.alpha)).clamp(1e-4, 1e4)
        self.smooth_scale.data = smooth.to(self.target_dtype)

        # W smoothing: W_smooth[i,j] = W[i,j] / smooth[j]
        w_smoothed = self.weight.float() / smooth.view(1, -1)

        # Group-wise RHT on weight rows (in_features axis)
        w_rot = group_rht(w_smoothed, self.H64.float(), N0=self.N0)

        # NVFP4 quantization of rotated weight
        self.w_quantized.copy_(
            quantize_to_nvfp4(w_rot, self.block_size).to(self.target_dtype)
        )
        self.is_calibrated = True
    ...
    def forward(self, x):
        ...
        # SmoothQuant on activation
        x_smoothed = x.to(self.target_dtype) * self.smooth_scale

        # Group-wise RHT on activation (in_features axis)
        x_rot = group_rht(x_smoothed, self.H64, N0=self.N0)

        # NVFP4 quantization of rotated activation
        x_q = quantize_to_nvfp4(x_rot, self.block_size)
...
def apply_convrot_quantization(pipe, transformer, accelerator, prompts, p_count,
                                t_count, device, args):
    """M8: ConvRot — Group-wise Regular Hadamard + SmoothQuant + NVFP4 W4A4."""
    target_names = get_target_linear_names(transformer)
    if accelerator.is_main_process:
        print(f"  [CONVROT] Targeting {len(target_names)} layers")

    print(f"  [CONVROT] Calibrating ({p_count} prompts)...", flush=True)
    calib_data = _collect_x_max(pipe, transformer, target_names,
                                 prompts, p_count, t_count, device, accelerator)

    for name in tqdm(target_names, desc="[CONVROT] Replacing layers",
                     disable=not accelerator.is_main_process):
        orig_m = get_module_by_name(transformer, name)
        if next(orig_m.parameters()).device == device:
    ...
    accelerator.wait_for_everyone()
    if accelerator.is_main_process:
        print("  [CONVROT] Quantization complete.")


# ---------------------------------------------------------------------------
# Utility: Strip SVDQuant internal LoRA correction
# ---------------------------------------------------------------------------

def strip_svdquant_lora(model):
    """SVDQuant 내부 rank-32 LoRA 버퍼를 제거하여 plain FP4 forward로 fall-through.

    SVDQuantTensorQuantizer는 _svdquant_lora_a / _svdquant_lora_b 버퍼로 LR 보정을 저장.
    svdquant_lora_a property가 hasattr 체크 → 없으면 None 반환.
    SVDQuantLinear.forward()에서 lora_a is None이면 plain FP4 forward 사용.

    주의: 양자화된 weight는 (original_weight - lora_b @ lora_a)를 FP4 양자화한 상태.
          LR 제거 시 forward = FP4(W - LR) @ x — RTN과 다른 조건.
    """
    count = 0
    for module in model.modules():
        if hasattr(module, '_svdquant_lora_a'):
            delattr(module, '_svdquant_lora_a')
            if hasattr(module, '_svdquant_lora_b'):
                delattr(module, '_svdquant_lora_b')
            count += 1
    print(f"  [SVDQuant] Stripped LoRA from {count} quantizer(s)")
