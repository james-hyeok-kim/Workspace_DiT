...
echo "  D-A rank=32 no-cache : $DEEP_DIR/interval1_s${CACHE_START}_e${CACHE_END}_gs4.5_steps${NUM_STEPS}/$DATASET/metrics.csv"
echo "  D-C rank=0  no-cache : $DEEP_DIR/interval1_s${CACHE_START}_e${CACHE_END}_gs4.5_steps${NUM_STEPS}/$DATASET/metrics.csv"
echo "  B   rank=32 cache    : (기존 조합1, FID=162.99)"
echo "  A-2 rank=0  cache    : $DEEP_DIR/interval${CACHE_INTERVAL}_s${CACHE_START}_e${CACHE_END}_gs4.5_steps${NUM_STEPS}/$DATASET/metrics.csv"
echo ""
echo "  → 4개 셀로 SVD 기여 / Cache 기여 / 상호작용 분리 가능"
echo ""
echo "[Exp A] Residual error (stale vs fresh residual):"
echo "  rank=32: $DEEP_DIR/interval${CACHE_INTERVAL}_s${CACHE_START}_e${CACHE_END}_gs4.5_steps${NUM_STEPS}/$DATASET/residual_errors_rank32.csv"
echo "  rank=0 : $DEEP_DIR/interval${CACHE_INTERVAL}_s${CACHE_START}_e${CACHE_END}_gs4.5_steps${NUM_STEPS}/$DATASET/residual_errors_rank0.csv"
echo ""
echo "[Exp B] Block drift profile:"
echo "  $DEEP_DIR/interval${CACHE_INTERVAL}_s${CACHE_START}_e${CACHE_END}_gs4.5_steps${NUM_STEPS}/$DATASET/block_drift_profile.csv"
echo ""
echo "[Exp C] Cache-aware calib vs standard:"
echo "  $DEEP_DIR/interval${CACHE_INTERVAL}_s${CACHE_START}_e${CACHE_END}_gs4.5_steps${NUM_STEPS}_calib_cache/$DATASET/metrics.csv"
