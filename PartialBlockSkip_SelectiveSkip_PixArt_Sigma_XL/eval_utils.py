...
def generate_and_evaluate(
    ...
    skipped for generation (pipe call, cache_state transitions, and timing are
    all omitted for cached samples).  time_per_image_sec reflects only the
    newly generated samples so the number is not diluted by cache hits.
    """
    ...
    with accelerator.split_between_processes(indices) as local_indices:
        for i in local_indices:
            ...
            if os.path.exists(save_path):
                # Reuse existing sample — skip pipe call and cache state transitions.
