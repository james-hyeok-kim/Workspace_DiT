...
MODES="gelu mlp res film"   # 실행할 nl 모드; gelu_fd / gelu_fdw / gelu_ts / gelu_traj 도 지원
...
if [[ " $MODES " == *" gelu_fd "* ]]; then
    echo ""
    echo "======= Option 5: GELU + FD (rank=$RANK) ======="
    for STEP in "${STEPS[@]}"; do
...
# ── Option 7: GELU-T + FD Stratified (Level 2: t-conditioned + bucket loss) ──
if [[ " $MODES " == *" gelu_ts "* ]]; then
    echo ""
    echo "======= Option 7: GELU-T + FD Stratified (rank=$RANK) ======="
    for STEP in "${STEPS[@]}"; do
        run_one "cache_nl_gelu_t" "$STEP" "" "fd_stratified"
    done
fi

# ── Option 8: GELU-T + Trajectory Distillation ──────────────────────────────
if [[ " $MODES " == *" gelu_traj "* ]]; then
    echo ""
    echo "======= Option 8: GELU-T + Trajectory Distillation (rank=$RANK) ======="
    for STEP in "${STEPS[@]}"; do
        run_one "cache_nl_gelu_t" "$STEP" "" "traj_distill"
    done
fi

# ── Option 9: GELU + Combined Drift+Trajectory Distillation ─────────────────
if [[ " $MODES " == *" gelu_dtraj "* ]]; then
    echo ""
    echo "======= Option 9: GELU + Combined Drift+Traj (rank=$RANK) ======="
    for STEP in "${STEPS[@]}"; do
        run_one "cache_nl_gelu" "$STEP" "" "drift_traj"
    done
fi
