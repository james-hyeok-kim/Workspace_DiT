...
def generate_and_evaluate(
    ...
    Image generation is distributed across GPUs (split_between_processes).
    Metric computation runs on main process only to avoid NCCL sync issues.
    Non-main processes wait at a final barrier while main computes metrics.

    If sample_{i}.png already exists in save_dir, it is loaded from disk and
    skipped for generation.  time_per_image_sec reflects only newly generated samples.
    """
    ...
    # All ranks sync after generation; non-main ranks then idle at next barrier
    accelerator.wait_for_everyone()

    # ── Metric computation: main process only ────────────────────────────────
    result = {}
    if accelerator.is_main_process:
        avg_time = float(np.mean(local_times)) if local_times else 0.0

        # sync_on_compute=False: disable NCCL all-gather inside compute().
        # Metrics are computed locally on main process; no cross-rank sync needed.
        ...
        is_m    = InceptionScore(sync_on_compute=False).to(device)
    ...
    # Final barrier: non-main ranks wait here while main computes metrics above.
    # Prevents NCCL watchdog timeout when main process does long local computation.
    accelerator.wait_for_everyone()
    return result
